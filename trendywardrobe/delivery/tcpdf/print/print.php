...
users.billingAddress as billingAddress,users.billingCity as billingCity,users.billingState as billingState,users.billingPincode as billingPincode,
...
To<br>
'.$row1['username'].'<br>
'.htmlentities($row1['billingAddress'].",".$row1['billingCity'].",".$row1['billingState']."-".$row1['billingPincode']).'<br>
...
orders.userId=users.id join products on products.id=orders.productId where orders.orderDate Between '$from' and '$to' and users.id=$user_id");
									...
									{
									    $new_price=$row['productprice']*$row['quantity'];
